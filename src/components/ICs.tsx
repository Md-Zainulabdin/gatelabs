        ...
        <div className="grid grid-cols-1 gap-2">
          {Object.keys(ICS).map((id) => (
            ...
            >
              ...
              <ChevronRight
                className={`w-4 h-4 ${selectedIC === id ? "opacity-100" : "opacity-0"}`}
              />
            </button>
          ))}
        </div>
        ...
        <div className="card">
          ...
          <div className="space-y-8">
            <section className="grid grid-cols-1 md:grid-cols-5 gap-4">
              {/* Pin Configration */}
              <div className="md:col-span-3 border border-zinc-100">
                <img
                  src={ic?.diagramUrl}
                  alt={ic?.name}
              ...
              <div className="md:col-span-2 bg-zinc-50 border border-zinc-100 p-8 flex justify-center items-center">
                <div className="relative w-48 h-64 bg-zinc-800 border-4 border-zinc-700 rounded-sm flex flex-col items-center justify-center">
                  {/* smaller: was w-64 h-96, now w-48 h-64 */}
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-zinc-50 border-b border-zinc-200">
                      <th className="px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                        Pin
                      </th>
                      <th className="px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-zinc-500">
                        Name
                    ...
                    {ic.pinout.map((pin) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-tighter rounded-sm ${
                              pin.type === "power"
